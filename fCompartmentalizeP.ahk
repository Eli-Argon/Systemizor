fCompartmentalizeP(dPanel, pOutputMainDir, pOutputExtraDir, aPanelsToSplit) {
	Local
	Global oLogger

	oXml := ComObjCreate("MSXML2.DOMDocument.6.0")
	oXml.async := false, oXml.preserveWhiteSpace := true																    
    oXml.loadXml(dPanel.sFileContents)
    
    fAbort(oXml.parseError.errorCode, A_ThisFunc, "Ошибка при чтении XML.", { "dPanel.pFile": dPanel.pFile
    , "oXml.parseError.errorCode": oXml.parseError.errorCode, "oXml.parseError.reason": oXml.parseError.reason })
    ...
    For oSteel in oXml.selectNodes("/" ns("PXML_Document","Order","Product","Slab","Steel")) {
        ...
        If (oSteel.getAttribute("Type") == "none") { ;===== vvv  An extra reinforcement node  vvv ===========;
            fAbort(hasExtra, A_ThisFunc, "Два комплекта усилении̌ в """ dPanel.sFileName """?", { "dPanel.pFile": dPanel.pFile }) 
            hasExtra := true, oSteelExtra := oSteel.parentNode.removeChild(oSteel)

            sExtraContents := fExtrasyzeP(oSteelExtra, dPanel.pFile)
        ...
        If (oSteel.getAttribute("Type") == "mesh") { ;====== vvv A mesh node vvv =====================;
            ...
            For oBar in oSteel.selectNodes(ns("Bar")) { ; Checking if mesh is bent.
                If (oBar.selectNodes(ns("Segment", "BendY")).length > 1) {
                    fAbort(hasBent, A_ThisFunc, "Две гнутых сетки в """ dPanel.sFileName """?", { "dPanel.pFile": dPanel.pFile })
                    isBent := true, hasBent := true, oSteelBent := oSteel
                    break
                }
            }
            If (!isBent) { ; If bent bars have not been found in this mesh.
                fAbort(hasFlat, A_ThisFunc, "Две плоских сетки в """ dPanel.sFileName """?", { "dPanel.pFile": dPanel.pFile })
    ...
    fAbort(nSteelNodes == 0, A_ThisFunc, "Нет сеток в source-файле «" dPanel.sFileName "»?", { "dPanel.pFile": dPanel.pFile }) 
    fAbort(nSteelNodes > 3, A_ThisFunc, "Три сетки в source-файле «" dPanel.sFileName "»?", { "dPanel.pFile": dPanel.pFile }) 
    ...
    If ( shouldSplit ) { ; ############### If the main file needs to be split in two ########################## ;
        fAbort(nSteelNodes == 1, A_ThisFunc, "«" dPanel.sName "» в списке «Split», но найдена только одна сетка."
        , { "dPanel.pFile": dPanel.pFile })

        oSteelFlat := oSteelFlat.parentNode.removeChild(oSteelFlat)
        oXml.save(pMainFile . "-BENT.pxml")
        ; fAbort(!FileExist(pMainFile . "-BENT.pxml"), A_ThisFunc, "Ошибка при создании """ pMainFile "-BENT"".")

        oSteelBent.parentNode.appendChild(oSteelFlat)
        oSteelBent.parentNode.removeChild(oSteelBent)
        oXml.save(pMainFile . "-FLAT.pxml")		
        ; fAbort(!FileExist(pMainFile . "-FLAT.pxml"), A_ThisFunc, "Ошибка при создании """ pMainFile "-FLAT"".")
...
; Extracts extra reinforcement data from a PXML Steel node. Sorts bars first by length, then by diameter. Returns XML string.
fExtrasyzeP(oSteelExtra, pFile) {
	Local
	sTemplateTop =
(
...
(
	...
	fAbort((oSteelExtra.getAttribute("Type") != "none"), A_ThisFunc, "Неверныи̌ атрибут ""Тип"".", { "pFile": pFile })

    ; ######################### Making a list of extra reinforcement ########################### ;
    dList := {} ; dList = { "2780-8": { length: 2780, diameter: 8, quantity: 2 }, "2780-6": { length: 2780, diameter: 6, quantity: 4 }, ... }
